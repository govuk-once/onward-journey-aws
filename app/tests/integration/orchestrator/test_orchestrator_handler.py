...
def create_mock_gateway(
    db_response, kb_response, crm_availability_response=None, crm_handoff_response=None
):
    """
        ...
        class MockResponse:
            def json(self):
                ...
                if "query_department_database" in tool_name:
                    ...
                    "check_chat_availability" in tool_name and crm_availability_response
                ):
                ...
                return {"result": {"content": [{"text": content_text}]}}
...
def test_knowledge_base_resolution(mock_gateway, llm_judges):
    ...
    mock_db_result = json.dumps(
        [
            mock_data.test_contact_passport_tracking,
            mock_data.test_contact_visa_application,
            mock_data.test_contact_uc_helpline,
        ]
    )
...
def test_kb_failure_routes_to_crm(mock_gateway, llm_judges):
    ...
    logger.info(
        "TEST: Agent resolves query with no Knowledge Base article found, initiates human connection"
    )

    # 0.1 Setup mock data
    mock_db_result = json.dumps(
        [
            mock_data.test_contact_hmrc,
            mock_data.test_contact_dwp,
            mock_data.test_contact_uc_helpline,
        ]
    )
    mock_kb_result = "ERROR: No knowledge base articles found."
    mock_crm_availability_result = (
        "Live chat is AVAILABLE. Estimated wait: under 1 minute."
    )
    mock_crm_handoff_result = (
        "SIGNAL: initiate_live_handoff {'connection_details' : 'test'}"
    )
    ...
    assert (
        "available right now" in response_lower_1 or "connect you" in response_lower_1
    )
...
def test_kb_not_relevant_routes_to_crm_live_chat(mock_gateway, llm_judges):
    ...
    logger.info(
        "TEST: Agent resolves query with non-relevant Knowledge Base article found, initiates human connection"
    )
...
def test_kb_not_relevant_routes_to_crm_no_live_chat(mock_gateway, llm_judges):
    ...
    logger.info(
        "TEST: Agent handles irrelevant Knowledge Base return and no live agents available - returns contact details"
    )
